...
class FullTokenizer(object):
    ...
    def __init__(self, vocab_file, model_file='', do_lower_case=True):
        ...
        self.sent_tokenizer = SentencePieceTokenizer(model_file, do_lower_case=do_lower_case)
        self.mecab_tokenizer = MecabTokenizer()
        self.vocab = load_vocab(vocab_file)
        self.inv_vocab = {v: k for k, v in self.vocab.items()}

    def tokenize(self, text):
        split_tokens = self.sent_tokenizer.tokenize(text)
        return split_tokens

    def space_separation(self, text):
        # MeCabで分かち書きをしたテキストを返す
        split_tokens = self.mecab_tokenizer.tokenize(text)
...
class MecabTokenizer(object):
    """Runs MeCab tokenization (from raw text to tokens list)"""

    def __init__(self, do_lower_case = True):
        """Constructs a MecabTokenizer"""
        self.tagger  =  MeCab.Tagger("-Owakati")
        self.do_lower_case = do_lower_case # 語彙中の英単語を小文字に変換するかどうか
    ...
    def tokenize(self, text):
        ...
        if self.do_lower_case:
            text = text.lower()
        output_tokens = self.tagger.parse(text).split()
        return output_tokens
